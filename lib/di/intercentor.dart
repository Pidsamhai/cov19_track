...
class CustomInterceptor extends Interceptor {
  ...
  void onRequest(
    ...
    if (level == Level1.none) return handler.next(options);

    logPrint('[DIO]--> ${options.method} ${options.uri}');

    if (level == Level1.basic) return handler.next(options);

    options.headers.forEach((key, value) {
      logPrint('[DIO][HEADER]$key:$value');
    });

    if (level == Level1.headers) {
  ...
  void onResponse(
    ...
    if (level == Level1.none) return handler.next(response);

    logPrint(
        '[DIO]<-- ${response.statusCode} ${(response.statusMessage?.isNotEmpty ?? false) ? response.statusMessage : ''}');

    if (level == Level1.basic) return handler.next(response);
  ...
  void onError(
    ...
    if (level == Level1.none) return handler.next(err);
